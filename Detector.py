...
def predict_image(image_path):
    ...
    vis_util.visualize_boxes_and_labels_on_image_array(
        ...
        category_index,
        min_score_thresh=.30, # IMPORTANT: Here we stablish a 30% of score to draw the inference boxes
    ...
    publish_result(output_dict["detection_scores"], os.path.abspath(image_path))
...
def detection_plot():
    for image_path in TEST_IMAGE_PATHS:
        ...
        Image.fromarray(image_np).convert('RGB').save(image_path) # Save the image result and overwrite the input one.
...
def publish_result(input, path):
    
    #If the score is 30% or more then publish the info
    if input.max() >= 0.30:
        ...
        tw.api.update_status(media_ids=[ret.media_id_string], status="INCENDIO DETECTADO")
        print("TWITTEADO OK")
